...
{
    FILE * file;
	if(!gDvm.processName || !strcmp(gDvm.processName,"zygote")) {
		char cmdline[256];
		cmdline[0] = NULL;
		...
		if(file) {
			...
			gDvm.processName = strdup(cmdline);
			fclose(file);
		}
	}
	if(gDvm.processName && strcmp(gDvm.processName,"zygote")) {
		file = fopen("/sdcard/toTrackPackage.txt" , "r");
		if(file) {
			fscanf(file,"%s",gDvm.toTrackAppProcess);
			fclose(file);
			file = fopen("/sdcard/toTrackDexFile.txt" , "r");                 
			uint32_t checksum;
			if (file) {
		    	if(fscanf(file, "%x", &checksum)!=EOF) {
		            gDvm.isSetupComplete = true;
		            gDvm.targetDexChecksum = checksum;
		            ALOGD("DVM_M3 Initialized, target check sum:%x, for process:%s\n",gDvm.targetDexChecksum,(gDvm.processName?gDvm.processName:"NULL"));
					if(!strcmp(gDvm.processName,gDvm.toTrackAppProcess)) {			
         ...
         if( method != NULL && !(method->traced) && method->clazz != NULL && method->clazz->pDvmDex != NULL) {  
                ...
                *isTraced = true;
                if(method->clazz->pDvmDex->pHeader->checksum == gDvm.targetDexChecksum) {
                    ALOGD("DVM_M3_Method:%s!%s",method->clazz->descriptor,method->name);    
                }                
        }
}
