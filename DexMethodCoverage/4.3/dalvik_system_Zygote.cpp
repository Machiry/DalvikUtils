...
static pid_t forkAndSpecializeCommon(const u4* args, bool isSystemServer)
{
    ...
    if (isSystemServer) {
        ...
        if (niceNameObj) {
			...
			ALOGE("DVM_M3: Unable to get the nice name");
		}
    }
    ...
    if (pid == 0) {
		...
		if(niceName) {
			ALOGE("DVM_M3: got process name:%s\n",niceName);
			gDvm.processName = strdup(niceName);
			if(gDvm.processName) {
				ALOGE("DVM_M3: Duplicated process name:%s\n",gDvm.processName);
			} else{
				ALOGE("DVM_M3: Unable to duplicate  process name:%s\n",niceName);
			}			
		}
